...
CREATE TRIGGER tr_cinema_before_insert BEFORE INSERT ON Cinema FOR EACH ROW
BEGIN
    ...
    IF NEW.contact_phone IS NOT NULL AND NEW.contact_phone != '' THEN
        IF NEW.contact_phone NOT REGEXP '^(1[3-9][0-9]{9}|0[0-9]{2,3}-?[0-9]{7,8})$' THEN
...
-- Cinema 更新前触发器：验证更新数据
CREATE TRIGGER tr_cinema_before_update BEFORE UPDATE ON Cinema FOR EACH ROW
BEGIN
    DECLARE cinema_count INT DEFAULT 0;
    DECLARE error_msg VARCHAR(255);
    ...
    SELECT COUNT(*) INTO cinema_count
    FROM Cinema
    WHERE name = NEW.name AND address = NEW.address AND cinema_id != NEW.cinema_id;
    IF cinema_count > 0 THEN
        SET error_msg = '同一地址的影院名称已存在';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_msg;
    END IF;
...
-- Cinema 更新后触发器：记录修改日志（注释掉，因为无 audit_log 表）
CREATE TRIGGER tr_cinema_after_update AFTER UPDATE ON Cinema FOR EACH ROW
BEGIN
    -- INSERT INTO audit_log (table_name, operation, record_id, old_values, new_values, update_time)
...
CREATE TRIGGER tr_hall_before_insert BEFORE INSERT ON Hall FOR EACH ROW
BEGIN
    ...
    -- 验证影厅名称
    IF NEW.hall_name IS NULL OR TRIM(NEW.hall_name) = '' THEN
    ...
    FROM Hall
    WHERE cinema_id = NEW.cinema_id AND hall_name = NEW.hall_name;
    ...
    -- 验证座位容量
    IF NEW.seat_count IS NULL OR NEW.seat_count <= 0 OR NEW.seat_count > 1000 THEN
        SET error_msg = '影厅容量必须在1-1000之间';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_msg;
    END IF;

    -- 验证影厅类型
    IF NEW.hall_type IS NOT NULL AND NEW.hall_type NOT IN ('2D', '3D', 'IMAX', '杜比', '4D', '动感', '巨幕') THEN
...
-- Hall 更新前触发器：验证更新数据
CREATE TRIGGER tr_hall_before_update BEFORE UPDATE ON Hall FOR EACH ROW
BEGIN
    DECLARE hall_count INT DEFAULT 0;
    DECLARE cinema_exists INT DEFAULT 0;
    ...
    SELECT COUNT(*) INTO hall_count
    FROM Hall
    WHERE cinema_id = NEW.cinema_id AND hall_name = NEW.hall_name AND hall_id != NEW.hall_id;
    IF hall_count > 0 THEN
        SET error_msg = '同一影院内影厅名称已存在';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_msg;
    END IF;
...
-- Hall 删除前触发器：检查是否有未来的排片
CREATE TRIGGER tr_hall_before_delete BEFORE DELETE ON Hall FOR EACH ROW
BEGIN
    DECLARE screening_count INT DEFAULT 0;
    -- 检查是否有未来的场次
    SELECT COUNT(*) INTO screening_count
    FROM Screening
    WHERE hall_id = OLD.hall_id AND screening_time > NOW();
    IF screening_count > 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = '该影厅还有未来的场次安排，无法删除';
    END IF;
END //
...
CREATE TRIGGER tr_movie_before_insert BEFORE INSERT ON Movie FOR EACH ROW
BEGIN
    ...
    FROM Movie
    WHERE title = NEW.title AND YEAR(release_date) = YEAR(NEW.release_date);
    ...
    -- 验证时长
    IF NEW.duration IS NULL OR NEW.duration <= 0 OR NEW.duration > 600 THEN
    ...
    -- 验证基础价格
    IF NEW.base_price IS NULL OR NEW.base_price <= 0 OR NEW.base_price > 1000 THEN
        SET error_msg = '基础价格必须在0.01-1000元之间';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_msg;
    END IF;

    -- 验证分类
    IF NEW.category IS NOT NULL AND CHAR_LENGTH(NEW.category) > 100 THEN
        SET error_msg = '电影分类长度不能超过100个字符';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_msg;
    END IF;

    -- 验证导演
    IF NEW.director IS NOT NULL AND CHAR_LENGTH(NEW.director) > 50 THEN
        SET error_msg = '导演信息长度不能超过50个字符';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_msg;
    END IF;

    -- 验证评分范围
    IF NEW.rating IS NOT NULL AND (NEW.rating < 1.0 OR NEW.rating > 5.0) THEN
...
-- Movie 更新前触发器：验证更新数据
CREATE TRIGGER tr_movie_before_update BEFORE UPDATE ON Movie FOR EACH ROW
BEGIN
    DECLARE movie_count INT DEFAULT 0;
    DECLARE future_screenings INT DEFAULT 0;
    DECLARE error_msg VARCHAR(255);
    ...
    SELECT COUNT(*) INTO movie_count
    FROM Movie
    WHERE title = NEW.title AND YEAR(release_date) = YEAR(NEW.release_date) AND movie_id != NEW.movie_id;
    IF movie_count > 0 THEN
        SET error_msg = '同年份已存在同名电影';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_msg;
    END IF;
    ...
    -- 如果提前撤档日期，检查是否有未来排片
    IF NEW.off_shelf_date IS NOT NULL AND (OLD.off_shelf_date IS NULL OR NEW.off_shelf_date < OLD.off_shelf_date) THEN
        SELECT COUNT(*) INTO future_screenings
        FROM Screening
        WHERE movie_id = NEW.movie_id AND screening_time > NOW() AND DATE(screening_time) > NEW.off_shelf_date;
        IF future_screenings > 0 THEN
...
-- Movie 更新后触发器：更新相关场次价格
CREATE TRIGGER tr_movie_after_update AFTER UPDATE ON Movie FOR EACH ROW
BEGIN
    IF OLD.base_price != NEW.base_price THEN
        UPDATE Screening
        SET ticket_price = NEW.base_price * (ticket_price / OLD.base_price)
        WHERE movie_id = NEW.movie_id AND screening_time > NOW();
    END IF;
END //
...
CREATE TRIGGER tr_user_before_insert BEFORE INSERT ON `User` FOR EACH ROW
BEGIN
    ...
    -- 验证密码强度
    IF NEW.password IS NULL OR NEW.password NOT REGEXP '^(?=.*[0-9])(?=.*[a-zA-Z])(?=.*[!@#$%^&*]).{8,100}$' THEN
        SET error_msg = '密码需包含数字、字母、特殊字符，且长度在8-100位之间';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_msg;
    END IF;

    -- 验证手机号格式和唯一性
    IF NEW.phone IS NOT NULL AND NEW.phone != '' THEN
        IF NEW.phone NOT REGEXP '^1[3-9][0-9]{9}$' THEN
...
-- User 更新前触发器：验证更新数据
CREATE TRIGGER tr_user_before_update BEFORE UPDATE ON `User` FOR EACH ROW
BEGIN
    DECLARE user_count INT DEFAULT 0;
    DECLARE error_msg VARCHAR(255);
...
-- User 更新后触发器：记录重要信息变更
CREATE TRIGGER tr_user_after_update AFTER UPDATE ON `User` FOR EACH ROW
BEGIN
    IF OLD.phone != NEW.phone OR OLD.email != NEW.email OR OLD.password != NEW.password THEN
        -- INSERT INTO security_log (user_id, operation, old_value, new_value, change_time)
...
CREATE TRIGGER tr_member_before_insert BEFORE INSERT ON Member FOR EACH ROW
BEGIN
    ...
    FROM Member
    WHERE user_id = NEW.user_id AND cinema_id = NEW.cinema_id;
...
-- Member 更新前触发器：验证更新数据
CREATE TRIGGER tr_member_before_update BEFORE UPDATE ON Member FOR EACH ROW
BEGIN
    DECLARE error_msg VARCHAR(255);
...
-- Member 更新后触发器：记录积分变化
CREATE TRIGGER tr_member_after_update AFTER UPDATE ON Member FOR EACH ROW
BEGIN
    IF OLD.points != NEW.points THEN
        -- INSERT INTO points_log (user_id, cinema_id, old_points, new_points, change_reason, change_time)
...
CREATE TRIGGER tr_screening_before_insert BEFORE INSERT ON Screening FOR EACH ROW
BEGIN
    ...
    -- 验证电影是否存在并获取信息
    SELECT COUNT(*), MAX(duration), MAX(base_price), MAX(release_date), MAX(off_shelf_date)
    INTO movie_exists, movie_duration, movie_base_price, movie_release_date, movie_end_date
    FROM Movie
    ...
    -- 验证影厅是否存在并获取影院信息
    SELECT COUNT(*), MAX(cinema_id), MAX(seat_count)
    INTO hall_exists, cinema_id_val, hall_capacity
    FROM Hall
    ...
    IF DATE(NEW.screening_time) < movie_release_date THEN
        ...
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_msg;
    END IF;
    IF movie_end_date IS NOT NULL AND DATE(NEW.screening_time) > movie_end_date THEN
    ...
    WHERE s.hall_id = NEW.hall_id
    AND (
        (NEW.screening_time >= s.screening_time AND NEW.screening_time < DATE_ADD(s.screening_time, INTERVAL (m.duration + 30) MINUTE))
        OR (DATE_ADD(NEW.screening_time, INTERVAL (movie_duration + 30) MINUTE) > s.screening_time
            AND DATE_ADD(NEW.screening_time, INTERVAL (movie_duration + 30) MINUTE) <= DATE_ADD(s.screening_time, INTERVAL (m.duration + 30) MINUTE))
        OR (NEW.screening_time <= s.screening_time
            AND DATE_ADD(NEW.screening_time, INTERVAL (movie_duration + 30) MINUTE) >= DATE_ADD(s.screening_time, INTERVAL (m.duration + 30) MINUTE))
    );
    ...
    -- 验证票价合理性
    IF NEW.ticket_price IS NOT NULL THEN
        IF NEW.ticket_price <= 0 OR NEW.ticket_price > 1000 THEN
...
-- Screening 插入后触发器：创建座位
CREATE TRIGGER tr_screening_after_insert AFTER INSERT ON Screening FOR EACH ROW
BEGIN
    DECLARE row_count INT DEFAULT 10; -- 假设每行10个座位，实际需根据影厅布局调整
    ...
    -- 为该场次创建座位
    WHILE i <= row_count AND NOT done DO
        SET j = 1;
        WHILE j <= col_count AND NOT done DO
            IF (i - 1) * col_count + j <= hall_capacity THEN
                INSERT INTO ScreeningSeat (screening_id, seat_row, seat_col, status, lock_time)
...
-- Screening 更新前触发器：验证更新操作
CREATE TRIGGER tr_screening_before_update BEFORE UPDATE ON Screening FOR EACH ROW
BEGIN
    ...
    FROM ScreeningSeat
    WHERE screening_id = NEW.screening_id AND status = '已售出';

    -- 如果有已售票，限制某些字段的修改
    IF sold_tickets > 0 THEN
        IF NEW.screening_time != OLD.screening_time THEN
    ...
    IF NEW.screening_time != OLD.screening_time THEN
        ...
        FROM Screening s
        JOIN Movie m ON s.movie_id = m.movie_id
        WHERE s.hall_id = NEW.hall_id AND s.screening_id != NEW.screening_id
        AND (
            (NEW.screening_time >= s.screening_time AND NEW.screening_time < DATE_ADD(s.screening_time, INTERVAL (m.duration + 30) MINUTE))
            OR (DATE_ADD(NEW.screening_time, INTERVAL (movie_duration + 30) MINUTE) > s.screening_time
                AND DATE_ADD(NEW.screening_time, INTERVAL (movie_duration + 30) MINUTE) <= DATE_ADD(s.screening_time, INTERVAL (m.duration + 30) MINUTE))
    ...
    -- 验证票价
    IF NEW.ticket_price IS NOT NULL AND (NEW.ticket_price <= 0 OR NEW.ticket_price > 1000) THEN
        SET error_msg = '票价必须在0.01-1000元之间';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_msg;
    END IF;
END //

-- Screening 删除前触发器：检查是否有已售票
CREATE TRIGGER tr_screening_before_delete BEFORE DELETE ON Screening FOR EACH ROW
BEGIN
    DECLARE sold_tickets INT DEFAULT 0;
    ...
    SELECT COUNT(*) INTO sold_tickets
    FROM ScreeningSeat
    WHERE screening_id = OLD.screening_id AND status = '已售出';
    IF sold_tickets > 0 THEN
    ...
    FROM ScreeningSeat
    WHERE screening_id = OLD.screening_id AND status = '已锁定';
...
CREATE TRIGGER tr_screening_seat_before_update BEFORE UPDATE ON ScreeningSeat FOR EACH ROW
BEGIN
    ...
    -- 验证状态转换的合法性
    IF OLD.status = '已售出' AND NEW.status != '已售出' THEN
        SET error_msg = '已售出的座位不能改为其他状态';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_msg;
    END IF;
    IF OLD.status = '可用' AND NEW.status = '已售出' THEN
    ...
    -- 设置锁定时间
    IF NEW.status = '已锁定' AND OLD.status != '已锁定' THEN
...
-- ScreeningSeat 更新后触发器：更新剩余座位数
CREATE TRIGGER tr_screening_seat_after_update AFTER UPDATE ON ScreeningSeat FOR EACH ROW
BEGIN
    ...
    IF OLD.status != NEW.status THEN
        ...
        FROM ScreeningSeat
        WHERE screening_id = NEW.screening_id AND status = '可用';
        UPDATE Screening
...
CREATE TRIGGER tr_group_ticket_type_before_insert BEFORE INSERT ON GroupTicketType FOR EACH ROW
BEGIN
    DECLARE cinema_exists INT DEFAULT 0;
    DECLARE movie_exists INT DEFAULT 0;
    ...
    WHERE cinema_id = NEW.cinema_id AND movie_id = NEW.movie_id AND ticket_type = NEW.ticket_type;
    ...
    -- 验证购买人数范围
    IF NEW.min_client_count IS NULL OR NEW.min_client_count <= 0 OR NEW.min_client_count > NEW.max_client_count THEN
...
-- GroupTicketType 更新前触发器：验证更新操作
CREATE TRIGGER tr_group_ticket_type_before_update BEFORE UPDATE ON GroupTicketType FOR EACH ROW
BEGIN
    DECLARE duplicate_count INT DEFAULT 0;
    ...
    SELECT COUNT(*) INTO duplicate_count
    FROM GroupTicketType
    WHERE cinema_id = NEW.cinema_id AND movie_id = NEW.movie_id AND ticket_type = NEW.ticket_type AND type_id != NEW.type_id;
    IF duplicate_count > 0 THEN
        SET error_msg = '该影院和电影的团体票类型已存在';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_msg;
    END IF;
    ...
    -- 如果过期，自动设置为无效
    IF NEW.valid_until <= NOW() AND NEW.is_active = 1 THEN
        SET NEW.is_active = 0;
    END IF;
END //
...
CREATE TRIGGER tr_purchased_group_ticket_before_insert BEFORE INSERT ON PurchasedGroupTicket FOR EACH ROW
BEGIN
    ...
    -- 验证团体票类型信息
    SELECT COUNT(*), valid_until, min_client_count, max_client_count, unit_price, cinema_id
    INTO ticket_exists, ticket_valid_until, min_client_count, max_client_count, unit_price_val, cinema_id_val
    FROM GroupTicketType
    WHERE type_id = NEW.type_id;
    ...
    -- 验证团体票状态
    IF (SELECT is_active FROM GroupTicketType WHERE type_id = NEW.type_id) != 1 THEN
    ...
    -- 验证购买数量
    IF NEW.ticket_count IS NULL OR NEW.ticket_count <= 0 THEN
...
-- PurchasedGroupTicket 插入后触发器：更新相关统计
CREATE TRIGGER tr_purchased_group_ticket_after_insert AFTER INSERT ON PurchasedGroupTicket FOR EACH ROW
BEGIN
    DECLARE cinema_id_val INT;
    DECLARE member_exists INT DEFAULT 0;
    ...
    -- 获取影院ID和单价
    SELECT cinema_id, unit_price INTO cinema_id_val, unit_price_val
    FROM GroupTicketType
    WHERE type_id = NEW.type_id;

    -- 检查用户是否是该影院的会员
    SELECT COUNT(*), COALESCE(points, 0) INTO member_exists, current_points
    FROM Member
    WHERE user_id = NEW.user_id AND cinema_id = cinema_id_val;
...
CREATE TRIGGER tr_order_before_insert BEFORE INSERT ON `Order` FOR EACH ROW
BEGIN
    DECLARE user_exists INT DEFAULT 0;
    ...
    -- 如果是常规订单，验证排片信息
    IF NEW.purchase_type = 'regular' AND NEW.screening_id IS NOT NULL THEN
        SELECT COUNT(*), screening_time INTO screening_exists, screening_time
        FROM Screening
        WHERE screening_id = NEW.screening_id;
    ...
    IF NEW.order_number IS NULL OR NEW.order_number = '' THEN
        SET NEW.order_number = CONCAT('ORD', DATE_FORMAT(NOW(), '%Y%m%d%H%i%s'), LPAD(FLOOR(RAND() * 10000), 4, '0'));
    END IF;
    ...
    -- 验证支付金额
    IF NEW.cost_number IS NULL OR NEW.cost_number <= 0 THEN
    ...
    -- 如果状态是已付，设置支付时间
    IF NEW.status = '已付' AND NEW.payment_time IS NULL THEN
        SET NEW.payment_time = NOW();
    END IF;
END //

-- Order 更新前触发器：验证状态变更
CREATE TRIGGER tr_order_before_update BEFORE UPDATE ON `Order` FOR EACH ROW
BEGIN
    DECLARE screening_time DATETIME;
    DECLARE error_msg VARCHAR(255);
    ...
    IF OLD.status != '已取消' AND NEW.status = '已取消' AND NEW.purchase_type = 'regular' AND NEW.screening_id IS NOT NULL THEN
        SELECT screening_time INTO screening_time
        FROM Screening
        WHERE screening_id = NEW.screening_id;
    ...
    -- 设置支付时间
    IF OLD.status != '已付' AND NEW.status = '已付' AND NEW.payment_time IS NULL THEN
        SET NEW.payment_time = NOW();
    END IF;
END //

-- Order 更新后触发器：处理订单状态变化
CREATE TRIGGER tr_order_after_update AFTER UPDATE ON `Order` FOR EACH ROW
BEGIN
    DECLARE cinema_id_val INT;
    DECLARE member_exists INT DEFAULT 0;
    DECLARE points_to_add INT DEFAULT 0;
    ...
    IF OLD.status != '已取消' AND NEW.status = '已取消' THEN
        IF NEW.purchase_type = 'regular' AND NEW.screening_id IS NOT NULL THEN
            ...
            SET status = '可用', lock_time = NULL
            WHERE screening_id = NEW.screening_id
            AND screening_seat_id IN (SELECT screening_seat_id FROM Verification WHERE order_id = NEW.order_id);
        END IF;
    END IF;

    -- 如果订单从待定变为已付，给会员增加积分
    IF OLD.status = '待定' AND NEW.status = '已付' THEN
        IF NEW.purchase_type = 'regular' AND NEW.screening_id IS NOT NULL THEN
...
CREATE TRIGGER tr_verification_before_insert BEFORE INSERT ON Verification FOR EACH ROW
BEGIN
    ...
    -- 验证订单是否存在且有效
    SELECT COUNT(*), status, user_id INTO order_exists, order_status, user_id_val
    FROM `Order`
    ...
    IF NEW.screening_seat_id IS NOT NULL THEN
        SELECT COUNT(*), status INTO screening_exists, seat_status
        FROM ScreeningSeat
    ...
    IF NEW.verification_code IS NULL OR NEW.verification_code = '' THEN
        SET NEW.verification_code = UPPER(CONCAT(DATE_FORMAT(NOW(), '%Y%m%d'), LPAD(user_id_val, 4, '0'), LPAD(FLOOR(RAND() * 10000), 4, '0')));
    END IF;
...
-- Verification 更新前触发器：验证核销操作
CREATE TRIGGER tr_verification_before_update BEFORE UPDATE ON Verification FOR EACH ROW
BEGIN
    DECLARE screening_time DATETIME;
    DECLARE seat_status VARCHAR(10);
    DECLARE error_msg VARCHAR(255);
    ...
    IF OLD.is_used = 0 AND NEW.is_used = 1 THEN
        IF NEW.screening_seat_id IS NOT NULL THEN
            SELECT s.screening_time, ss.status INTO screening_time, seat_status
    ...
    -- 验证已使用的验证码不能改回未使用
    IF OLD.is_used = 1 AND NEW.is_used = 0 THEN
        SET error_msg = '已使用的验证码不能改回未使用状态';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_msg;
    END IF;
END //

-- Verification 更新后触发器：处理核销后续操作
CREATE TRIGGER tr_verification_after_update AFTER UPDATE ON Verification FOR EACH ROW
BEGIN
    IF OLD.is_used = 0 AND NEW.is_used = 1 THEN
        IF NEW.screening_seat_id IS NOT NULL THEN
            UPDATE ScreeningSeat
            SET status = '已售出', lock_time = NULL
            WHERE screening_seat_id = NEW.screening_seat_id;
        END IF;
    END IF;
END //
...
CREATE TRIGGER tr_review_before_insert BEFORE INSERT ON Review FOR EACH ROW
BEGIN
    DECLARE user_exists INT DEFAULT 0;
    DECLARE movie_exists INT DEFAULT 0;
    DECLARE screening_exists INT DEFAULT 0;
    ...
    WHERE o.user_id = NEW.user_id AND o.screening_id = NEW.screening_id AND o.status = '已付' AND v.is_used = 1;
    ...
    FROM Review
    WHERE user_id = NEW.user_id AND screening_id = NEW.screening_id;
    ...
    -- 验证评论内容
    IF NEW.comment IS NOT NULL AND LENGTH(TRIM(NEW.comment)) > 1000 THEN
...
-- Review 插入后触发器：更新电影评分
CREATE TRIGGER tr_review_after_insert AFTER INSERT ON Review FOR EACH ROW
BEGIN
    DECLARE avg_rating DECIMAL(3, 1);
    DECLARE review_count INT DEFAULT 0;

    -- 只有当新评论状态为'批准'且有评分时才更新电影评分
    IF NEW.status = '批准' AND NEW.rating IS NOT NULL THEN
        SELECT AVG(rating), COUNT(*) INTO avg_rating, review_count
        FROM Review
        WHERE movie_id = NEW.movie_id AND status = '批准' AND rating IS NOT NULL;
        UPDATE Movie
...
-- Review 更新前触发器：验证评论更新
CREATE TRIGGER tr_review_before_update BEFORE UPDATE ON Review FOR EACH ROW
BEGIN
    DECLARE error_msg VARCHAR(255);
...
-- Review 更新后触发器：更新电影评分
CREATE TRIGGER tr_review_after_update AFTER UPDATE ON Review FOR EACH ROW
BEGIN
    DECLARE avg_rating DECIMAL(3, 1);
    DECLARE review_count INT DEFAULT 0;

    -- 如果评分或状态发生变化，更新电影评分
    IF (OLD.rating != NEW.rating OR OLD.status != NEW.status) AND NEW.status = '批准' AND NEW.rating IS NOT NULL THEN
        SELECT AVG(rating), COUNT(*) INTO avg_rating, review_count
        FROM Review
        WHERE movie_id = NEW.movie_id AND status = '批准' AND rating IS NOT NULL;
        UPDATE Movie
    ...
    -- 如果状态从批准变为非批准，重新计算评分
    IF OLD.status = '批准' AND NEW.status != '批准' THEN
        SELECT AVG(rating), COUNT(*) INTO avg_rating, review_count
        FROM Review
        WHERE movie_id = NEW.movie_id AND status = '批准' AND rating IS NOT NULL;
...
CREATE TRIGGER tr_refund_before_insert BEFORE INSERT ON Refund FOR EACH ROW
BEGIN
    DECLARE order_exists INT DEFAULT 0;
    DECLARE order_status VARCHAR(10);
    DECLARE screening_time DATETIME;
    ...
    -- 验证订单是否存在
    SELECT COUNT(*), status, purchase_type, screening_id INTO order_exists, order_status, purchase_type, screening_time
    FROM `Order`
    WHERE order_id = NEW.order_id;
    IF order_exists = 0 THEN
        SET error_msg = '指定的订单不存在';
    ...
    -- 对于常规订单，检查是否在开场前30分钟
    IF purchase_type = 'regular' AND screening_time IS NOT NULL THEN
        SELECT screening_time INTO screening_time
        FROM Screening
    ...
    -- 验证退款方式
    IF NEW.refund_method IS NOT NULL AND NEW.refund_method NOT IN ('银行卡', '支付宝', '微信') THEN
...
-- Refund 更新前触发器：验证状态变更
CREATE TRIGGER tr_refund_before_update BEFORE UPDATE ON Refund FOR EACH ROW
BEGIN
    DECLARE error_msg VARCHAR(255);
    ...
    IF OLD.status = '已完成' AND NEW.status != '已完成' THEN
        ...
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_msg;
    END IF;
    IF OLD.status = '失败' AND NEW.status != '失败' THEN
...
-- Refund 更新后触发器：处理退款状态变化
CREATE TRIGGER tr_refund_after_update AFTER UPDATE ON Refund FOR EACH ROW
BEGIN
    DECLARE cinema_id_val INT;
    DECLARE member_exists INT DEFAULT 0;
    ...
    IF OLD.status != '已完成' AND NEW.status = '已完成' THEN
        ...
        -- 释放座位
        IF (SELECT purchase_type FROM `Order` WHERE order_id = NEW.order_id) = 'regular' THEN
            UPDATE ScreeningSeat
            SET status = '可用', lock_time = NULL
        ...
        -- 扣除会员积分
        SELECT o.user_id, o.purchase_type, o.screening_id, o.purchased_ticket_id
        INTO @user_id, @purchase_type, @screening_id, @purchased_ticket_id
        FROM `Order` o
        WHERE o.order_id = NEW.order_id;

        IF @purchase_type = 'regular' AND @screening_id IS NOT NULL THEN
            SELECT c.cinema_id INTO cinema_id_val
            FROM Screening s
            JOIN Hall h ON s.hall_id = h.hall_id
            JOIN Cinema c ON h.cinema_id = c.cinema_id
            WHERE s.screening_id = @screening_id;
        ELSEIF @purchase_type = 'group' AND @purchased_ticket_id IS NOT NULL THEN
            SELECT gtt.cinema_id INTO cinema_id_val
            FROM PurchasedGroupTicket pgt
            JOIN GroupTicketType gtt ON pgt.type_id = gtt.type_id
        ...
        IF cinema_id_val IS NOT NULL THEN
            SELECT COUNT(*) INTO member_exists
            FROM Member
            WHERE user_id = @user_id AND cinema_id = cinema_id_val;
            IF member_exists > 0 THEN
...
-- =============================================
-- 清理过期的座位锁定
CREATE EVENT IF NOT EXISTS clear_expired_locks
...
BEGIN
    ...
    UPDATE ScreeningSeat
    SET status = '可用', lock_time = NULL
    WHERE status = '已锁定' AND lock_time < DATE_SUB(NOW(), INTERVAL 15 MINUTE);
...
-- 清理过期的团体票类型
CREATE EVENT IF NOT EXISTS clear_expired_group_tickets
ON SCHEDULE EVERY 1 DAY
DO
BEGIN
    UPDATE GroupTicketType
    SET is_active = 0
    WHERE valid_until < NOW() AND is_active = 1;
...
-- 自动更新电影评分
CREATE EVENT IF NOT EXISTS update_movie_ratings
...
BEGIN
    ...
    OPEN cur;
    read_loop: LOOP
        FETCH cur INTO movie_id_val;
        IF done THEN
            LEAVE read_loop;
        END IF;
        SELECT AVG(rating), COUNT(*) INTO avg_rating, review_count
        FROM Review
        WHERE movie_id = movie_id_val AND status = '批准' AND rating IS NOT NULL;
        IF review_count > 0 THEN
            UPDATE Movie
            SET rating = ROUND(avg_rating, 1)
